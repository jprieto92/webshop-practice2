...
public class Controller {
	...
	public List<Usuario> usuarios(@RequestParam(value="productId", required=false) Integer productId,
			...
			@RequestParam(value="idTipoUsuario", required=false) Integer idTipoUsuario) throws NotFoundException{
		...
		if(terminoFiltrado != null){
			...
			if(usuarios==null)throw new NotFoundException("No existen usuarios con el nombre: "+terminoFiltrado);
		...
		/* Si hemos recibido el parámetro, se realizará una búsqueda filtrada por el id de Producto */
			...
			if(usuarios==null)throw new NotFoundException("No existe un usuario que tenga asociado el id de producto: "+productId);
			...
			if(usuarios==null)throw new NotFoundException("No existen usuarios con el id tipo usuario: "+idTipoUsuario);
	...
	public Usuario usuarioPorEmail(@PathVariable("email") String email) throws NotFoundException{
		System.out.println("Busca usuario por email"+email);
		Usuario usuario = usuarioRepository.findOne(email);

		if(usuario==null)throw new NotFoundException("No existe el usuario con el id: "+email);
	...
	public Usuario crear(@RequestBody Usuario usuario) throws IOException{
...
//		System.out.println("El telefono en el microservicio es: "+usuario.getTelefono());
		...
		try {
			...
			// Si lo recupera, es que existe. Conflicto, http 409
			throw new IOException("El usuario con email "+usuario.getEmail()+" ya existe.");
		} catch (NotFoundException e) {
			//Si no existe, es que se puede crear el usuario correctamente.
		}
	...
	public Usuario login(@RequestBody UsuarioLogin usuarioLogin) throws NotFoundException{
		...
		// Se lanza error HTTP 404
		if(usuarioReturn==null)throw new NotFoundException("Usuario y/o contraseña incorrectos");
	...
	public Usuario modificar(@RequestBody Usuario modificacionesUsuario) throws NotFoundException{
	...
	public void eliminar(@PathVariable("email") String email) throws NotFoundException{
		System.out.println("Eliminar producto con id: "+email);

//		if(email==null){
//			throw new NotFoundException("Debe indicar un email de usuario");
//		}
